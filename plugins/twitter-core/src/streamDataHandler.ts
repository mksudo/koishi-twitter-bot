...
export const registerStreamDataHandler = (
  ...
  if (ctx.twitterApi.isDataHandlerRegistered) {
    logger.debug("data handler already registered, skipping");
    return;
  }

  ctx.twitterApi.twitterStream.on(ETwitterStreamEvent.Data, async (tweet) => {
    logger.debug(`bots: ${JSON.stringify(ctx.bots.map((bot) => bot.selfId))}`);
    ...
    const screenshotResult: ITaskContext = await ctx.twitterHandler.screenshot(
      url
    );
    ...
    for (const userConfig of registeredUserConfigs) {
      ...
      await bot.sendMessage(userConfig.registeredBy, currText).catch((err) => {
        logger.warn(`send tweet to group ${userConfig.registeredBy} failed`);
      });
    }
  });

  ctx.twitterApi.isDataHandlerRegistered = true;
};
